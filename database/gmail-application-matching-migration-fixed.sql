-- Gmail Application Matching Migration Script (Fixed)
-- This safely adds email tracking fields to job_applications and creates related tables

-- 1. Safely add email-related fields to job_applications table (only if they don't exist)
DO $$
BEGIN
    -- Add latest_email_id if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='job_applications' AND column_name='latest_email_id') THEN
        ALTER TABLE public.job_applications ADD COLUMN latest_email_id TEXT;
    END IF;
    
    -- Add latest_email_subject if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='job_applications' AND column_name='latest_email_subject') THEN
        ALTER TABLE public.job_applications ADD COLUMN latest_email_subject TEXT;
    END IF;
    
    -- Add latest_email_from if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='job_applications' AND column_name='latest_email_from') THEN
        ALTER TABLE public.job_applications ADD COLUMN latest_email_from TEXT;
    END IF;
    
    -- Add latest_email_date if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='job_applications' AND column_name='latest_email_date') THEN
        ALTER TABLE public.job_applications ADD COLUMN latest_email_date TIMESTAMP WITH TIME ZONE;
    END IF;
    
    -- Add latest_email_snippet if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='job_applications' AND column_name='latest_email_snippet') THEN
        ALTER TABLE public.job_applications ADD COLUMN latest_email_snippet TEXT;
    END IF;
    
    -- Add status_updated_at if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='job_applications' AND column_name='status_updated_at') THEN
        ALTER TABLE public.job_applications ADD COLUMN status_updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
    
    -- Add email_count if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='job_applications' AND column_name='email_count') THEN
        ALTER TABLE public.job_applications ADD COLUMN email_count INTEGER DEFAULT 0;
    END IF;
END
$$;

-- 2. Create application_emails junction table (only if it doesn't exist)
...
-- 3. Create application status history table (only if it doesn't exist)
...
-- 5. Create unique constraint to prevent duplicate email processing (only if it doesn't exist)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'idx_application_emails_unique_message') THEN
        CREATE UNIQUE INDEX idx_application_emails_unique_message 
...
DO $$
BEGIN
    -- Enable RLS on application_emails if not already enabled
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'application_emails' AND rowsecurity = true) THEN
        ALTER TABLE public.application_emails ENABLE ROW LEVEL SECURITY;
    END IF;
    
    -- Enable RLS on application_status_history if not already enabled
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'application_status_history' AND rowsecurity = true) THEN
        ALTER TABLE public.application_status_history ENABLE ROW LEVEL SECURITY;
    END IF;
END
$$;

-- 7. Create RLS policies for application_emails (only if they don't exist)
DO $$
BEGIN
    -- Policy for SELECT
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'application_emails' AND policyname = 'Users can view their own application emails') THEN
        CREATE POLICY "Users can view their own application emails" ON public.application_emails
            FOR SELECT USING (auth.uid()::text = user_id);
    END IF;
    
    -- Policy for INSERT
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'application_emails' AND policyname = 'Users can insert their own application emails') THEN
        CREATE POLICY "Users can insert their own application emails" ON public.application_emails
            FOR INSERT WITH CHECK (auth.uid()::text = user_id);
    END IF;
    
    -- Policy for UPDATE
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'application_emails' AND policyname = 'Users can update their own application emails') THEN
...
-- 8. Create RLS policies for application_status_history (only if they don't exist)
DO $$
BEGIN
    -- Policy for SELECT
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'application_status_history' AND policyname = 'Users can view their own application status history') THEN
        CREATE POLICY "Users can view their own application status history" ON public.application_status_history
            FOR SELECT USING (auth.uid()::text = user_id);
    END IF;
    
    -- Policy for INSERT
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'application_status_history' AND policyname = 'Users can insert their own application status history') THEN
...
-- 9. Create function to update application status with history tracking (replace if exists)
...
-- 10. Create function to match email to application (replace if exists)
...
-- 11. Create or replace the applications with emails view
CREATE OR REPLACE VIEW public.applications_with_emails AS
SELECT 
    ja.*,
    COALESCE(ae.email_count, 0) as email_count,
    ae.latest_email_subject,
    ae.latest_email_from,
    ae.latest_email_date,
    ae.latest_email_snippet,
    -- Status history count
    COALESCE(ash.status_changes_count, 0) as status_changes_count,
    -- Latest status change
    ash.last_status_change
...
LEFT JOIN (
    SELECT 
        ...
        MAX(changed_at) as last_status_change
    FROM public.application_status_history
    GROUP BY application_id
) ash ON ja.id = ash.application_id;
